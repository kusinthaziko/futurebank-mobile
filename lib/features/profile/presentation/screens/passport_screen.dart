  ...
  Widget build(BuildContext context) => ListView(
    ...
    children: [
      FBCard(gradient: true, child: Column(children: [
        ...
        Text('MUBAS', style: AppTextStyles.titleLarge.copyWith(color: white)),
        const SizedBox(height: sp4),
        Text('Building since ${_formatDate(hs.accountAgeDays)}',
      ...
      FBCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _StatRow('Loan Repayment', '${(hs.loanRepaymentRate * 100).toInt()}%'),
      ...
      FBCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: sp12),
        const _MilestoneTile(icon: Icons.person_add, title: 'Account Created',
            subtitle: 'First step on your financial journey'),
        const _MilestoneTile(icon: Icons.savings, title: 'First Deposit',
            subtitle: 'Started building your savings habit'),
        if (hs.challengeCompletions > 0)
          const _MilestoneTile(icon: Icons.emoji_events, title: 'First Badge',
              subtitle: 'Earned your first achievement'),
        if ((hs.loanRepaymentRate) > 0)
          const _MilestoneTile(icon: Icons.credit_score, title: 'First Loan Repaid',
...
class _MilestoneTile extends StatelessWidget {
  ...
  const _MilestoneTile({required this.icon, required this.title, required this.subtitle});
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text(subtitle, style: AppTextStyles.caption.copyWith(color: gray500)),
      ])),
    ]),
  );
}
...
class _CredentialsTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) => badgesAsync.when(
    loading: () => const Center(child: CircularProgressIndicator()),
    error: (_, __) => Center(
      child: Text('Could not load badges',
                  ...
                  child: Row(children: [
                    ...
                    Expanded(child: Column(
                      ...
                      children: [
                        ...
                        Text('Token ID: #${b.badgeId.substring(0, 8)}',
  ...
  Widget build(BuildContext context) => ListView(
    ...
    children: [
      FBCard(child: Column(children: [
        ...
        const SizedBox(height: sp8),
        Text('Anyone can scan this QR code to verify your financial record directly on the blockchain — no middleman, no paperwork.',
            style: AppTextStyles.bodyMedium.copyWith(color: gray500),
            textAlign: TextAlign.center),
      ])),
      const SizedBox(height: sp16),
      if (did != null)
        FBCard(outlined: true, child: Column(children: [
          Row(children: [
      ...
      FBCard(outlined: true, child: Column(children: [
        const Text('What is a DID?',
            style: AppTextStyles.labelLarge),
        const SizedBox(height: sp8),
        Text('A Decentralized Identifier (DID) is your unique blockchain identity. Unlike a traditional ID, it belongs to you — not any institution — and travels with you after graduation.',
            style: AppTextStyles.bodyMedium.copyWith(color: gray500)),
      ])),
    ],
  );
