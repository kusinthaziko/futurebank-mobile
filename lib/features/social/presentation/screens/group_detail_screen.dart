  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: async.when(
        ...
        error: (e, _) => ErrorView(error: e, onRetry: () => ref.refresh(groupDetailProvider(groupId))),
...
class _GroupDetailBody extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final progress = goalAmount > 0 ? (poolAmount / goalAmount).clamp(0.0, 1.0) : 0.0;

    return ListView(
      padding: const EdgeInsets.all(sp16),
      children: [
        FBCard(gradient: true, child: Column(children: [
          Text(group.name, style: AppTextStyles.displayMedium.copyWith(color: white)),
          const SizedBox(height: sp4),
          Text('${group.groupType} · ${group.memberCount} members',
              style: AppTextStyles.bodyMedium.copyWith(color: white.withValues(alpha: 0.8))),
          const SizedBox(height: sp12),
          Text('MWK ${_formatAmount(group.poolBalance)} pooled',
        ...
        if (goalAmount > 0) ...[
          FBCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Progress to goal', style: AppTextStyles.titleMedium),
            ...
            const SizedBox(height: sp4),
            Text('MWK ${_formatAmount(group.poolBalance)} / MWK ${_formatAmount(group.goal!)}  ${(progress * 100).toInt()}%',
        ...
        FBCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (group.inviteCode != null) ...[
            Center(
              child: QrImageView(data: group.inviteCode!, size: 100,
          ...
          Row(children: [
            Text(group.inviteCode ?? '---',
                style: AppTextStyles.titleLarge.copyWith(letterSpacing: 4, color: primary500)),
            const Spacer(),
            IconButton(
        ...
        if (group.topContributors.isNotEmpty) ...[
          FBCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Top Contributors', style: AppTextStyles.titleMedium),
            const SizedBox(height: sp12),
            ...List.generate(group.topContributors.length, (i) {
              ...
              return Padding(
                ...
                child: Row(children: [
                  Text('${i + 1}.', style: AppTextStyles.labelLarge.copyWith(color: gray500)),
                  const SizedBox(width: sp8),
                  Expanded(child: Text(c.fullName, style: AppTextStyles.bodyMedium)),
                  Text('MWK ${_formatAmount(c.totalContributed)}',
        ...
        if (group.recentActivity.isNotEmpty) ...[
          FBCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: sp12),
            ...group.recentActivity.map((a) => Padding(
              padding: const EdgeInsets.symmetric(vertical: sp4),
              child: Row(children: [
        ...
        actions: [
          ...
          FBButton(
            ...
            onPressed: () async {
              ...
              Navigator.pop(ctx);
              try {
                await ref.read(socialRepositoryProvider(ref.read(accessTokenProvider)))
