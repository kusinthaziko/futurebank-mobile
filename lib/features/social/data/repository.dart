...
class SocialRepository {
  ...
  Future<({List<GroupModel> groups, List<ChallengeModel> challenges, List<LeaderboardEntry> leaderboard})>
    ...
    final cached = await _cacheService.getFreshValue<({List<GroupModel> groups, List<ChallengeModel> challenges, List<LeaderboardEntry> leaderboard})>(
      'social', institutionId,
      (json) => (
        groups: (json['groups'] as List).cast<Map<String, dynamic>>().map((g) => GroupModel.fromJson(g)).toList(),
        challenges: (json['challenges'] as List).cast<Map<String, dynamic>>().map((c) => ChallengeModel.fromJson(c)).toList(),
        leaderboard: (json['leaderboard'] as List).cast<Map<String, dynamic>>().map((e) => LeaderboardEntry.fromJson(e)).toList(),
      ),
    );
    ...
    try {
      ...
      final groups = (r.data!['myGroups'] as List).cast<Map<String, dynamic>>()
      ...
      final challenges = (r.data!['activeChallenges'] as List).cast<Map<String, dynamic>>()
      ...
      final leaderboard = (r.data!['leaderboard'] as List).cast<Map<String, dynamic>>()
          .map((e) => LeaderboardEntry.fromJson({'userId': e['user_id'],
              'fullName': e['full_name'], 'score': e['score'], 'rank': e['rank']})).toList();

      await _cacheService.cacheJson('social', institutionId, jsonEncode({
      ...
      final stale = await _cacheService.getStaleValue<({List<GroupModel> groups, List<ChallengeModel> challenges, List<LeaderboardEntry> leaderboard})>(
        'social', institutionId,
        (json) => (
          groups: (json['groups'] as List).cast<Map<String, dynamic>>().map((g) => GroupModel.fromJson(g)).toList(),
          challenges: (json['challenges'] as List).cast<Map<String, dynamic>>().map((c) => ChallengeModel.fromJson(c)).toList(),
  ...
  Future<GroupDetailModel> fetchGroupDetail(String groupId) async {
    ...
    try {
      ...
      if (r.hasException) throw r.exception!;
      final detail = GroupDetailModel.fromJson(r.data!['groupDetail'] as Map<String, dynamic>);

      await _cacheService.cacheJson('social', 'group_$groupId', jsonEncode({
        'id': detail.id, 'name': detail.name, 'status': detail.status,
        'invite_code': detail.inviteCode, 'pool_account_id': detail.poolAccountId,
        'pool_balance': detail.poolBalance, 'member_count': detail.memberCount,
        'goal': detail.goal, 'group_type': detail.groupType,
      }));
  ...
  Future<ChallengeDetailModel> fetchChallengeDetail(String challengeId) async {
    ...
    try {
      ...
      if (r.hasException) throw r.exception!;
      final detail = ChallengeDetailModel.fromJson(r.data!['challengeDetail'] as Map<String, dynamic>);

      await _cacheService.cacheJson('social', 'challenge_$challengeId', jsonEncode({
        'id': detail.id, 'title': detail.title, 'description': detail.description,
        'challenge_type': detail.challengeType, 'reward_points': detail.rewardPoints,
        'status': detail.status, 'ends_at': detail.endsAt,
        'participant_count': detail.participantCount,
        'current_progress': detail.currentProgress, 'target_progress': detail.targetProgress,
  ...
  Future<List<LeaderboardEntry>> fetchLeaderboard(String institutionId, String period) async {
    ...
    final cached = await _cacheService.getFreshValue<List<LeaderboardEntry>>(
      'social', key,
      (json) => (json['items'] as List).cast<Map<String, dynamic>>().map(LeaderboardEntry.fromJson).toList(),
    );
    if (cached != null) return cached;
    ...
    try {
      ...
      if (r.hasException) throw r.exception!;
      final entries = (r.data!['leaderboard'] as List).cast<Map<String, dynamic>>()
          .map((e) => LeaderboardEntry.fromJson({'userId': e['user_id'],
              'fullName': e['full_name'], 'score': e['score'], 'rank': e['rank']})).toList();

      await _cacheService.cacheJson('social', key, jsonEncode({
